...
# SET(CMAKE_C_FLAGS_DEBUG "-Og -g3 -DDEBUG" CACHE INTERNAL "c compiler flags debug")
# SET(CMAKE_CXX_FLAGS_DEBUG "-Og -g3 -DDEBUG" CACHE INTERNAL "cxx compiler flags debug")
...
SET(CMAKE_ASM_FLAGS_DEBUG_INIT "-g -DDEBUG")
...
SET(CMAKE_C_FLAGS_RELEASE_INIT "-Os -flto -DNDEBUG")
SET(CMAKE_CXX_FLAGS_RELEASE_INIT "-Os -flto -DNDEBUG")
SET(CMAKE_ASM_FLAGS_RELEASE_INIT "-DNDEBUG")
...
FUNCTION(STM32_ADD_DUMP_TARGET TARGET)
    ADD_CUSTOM_TARGET(${TARGET}.dump
        DEPENDS ${TARGET}
        COMMAND ${CMAKE_OBJDUMP} -x -D -S -s "$<TARGET_FILE:${TARGET}>" | ${CMAKE_CPPFILT} > "$<TARGET_FILE_DIR:${TARGET}>/${TARGET}.dump")
ENDFUNCTION()
...
FUNCTION(STM32_SET_FLASH_PARAMS TARGET STM32_FLASH_SIZE STM32_RAM_SIZE STM32_CCRAM_SIZE STM32_MIN_STACK_SIZE STM32_MIN_HEAP_SIZE STM32_FLASH_ORIGIN STM32_RAM_ORIGIN STM32_CCRAM_ORIGIN)
    SET(STM32_LINKER_SCRIPT_DEST)
    IF(NOT STM32_LINKER_SCRIPT)
        MESSAGE(STATUS "No linker script specified, generating default")
        INCLUDE(stm32_linker)
        SET(STM32_LINKER_SCRIPT_DEST "${CMAKE_CURRENT_BINARY_DIR}/${TARGET}_flash.ld")
        FILE(WRITE "${STM32_LINKER_SCRIPT_DEST}" ${STM32_LINKER_SCRIPT_TEXT})
    ELSE()
        FOREACH(LINKERSCRIPT ${STM32_LINKER_SCRIPT})
            get_filename_component(LINKERSCRIPT_NAME "${LINKERSCRIPT}" NAME_WE)
            set(LINKERSCRIPT_DEST "${CMAKE_CURRENT_BINARY_DIR}/${TARGET}_${LINKERSCRIPT_NAME}_flash.ld")
            CONFIGURE_FILE("${LINKERSCRIPT}" "${LINKERSCRIPT_DEST}")
            list(APPEND STM32_LINKER_SCRIPT_DEST "${LINKERSCRIPT_DEST}")
        ENDFOREACH()
    ENDIF()

    SET(EXTRA_LINKER_FLAGS)
    FOREACH(LINKERSCRIPT ${STM32_LINKER_SCRIPT_DEST})
        SET(EXTRA_LINKER_FLAGS "${EXTRA_LINKER_FLAGS} \"-T${LINKERSCRIPT}\"")
    ENDFOREACH()

    GET_TARGET_PROPERTY(TARGET_LD_FLAGS ${TARGET} LINK_FLAGS)
    IF(TARGET_LD_FLAGS)
        SET(TARGET_LD_FLAGS "${EXTRA_LINKER_FLAGS} ${TARGET_LD_FLAGS}")
    ELSE()
        SET(TARGET_LD_FLAGS "${EXTRA_LINKER_FLAGS}")
    ENDIF()
